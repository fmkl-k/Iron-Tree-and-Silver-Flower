...
void setup() {
  ...
  //初始化触摸传感器
  pinMode(KEYone, INPUT); //设置触摸传感器的连接管脚为输入模式
...
void loop() {
  ...
  unsigned long now = millis();             //当前时间(ms)
    ...
    float atotal =  sqrt(accx*accx + accy*accy + accz*accz);       //合成加速度
  ...
  Serial.print(atotal);Serial.println();
